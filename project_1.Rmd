...
# 7. Сomparison of the diameter of a mussels with the number of rings 5 and 15
...
We see that the distribution of the quantity is not normal, but the size of our sample is large enough (53 for Rings = 5 and 52 for Rings = 5). So, we can use t-test to test the following assumption: *the diameter of the 15-ring mussels is greater than the diameter of the 5-ring mussels.*

```{r t.test}
t.test(Diameter ~ Rings, data = df)
```
